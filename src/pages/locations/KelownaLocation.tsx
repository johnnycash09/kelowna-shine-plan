...
const slug = "/auto-detailing-kelowna";
const title = "Auto Detailing Kelowna BC | Santos Auto Detailing";
const description =
  "Santos Auto Detailing provides premium mobile detailing, ceramic coating, paint correction, boat detailing, aircraft detailing, and fleet detailing in Kelowna, BC.";
const canonical = `${SITE_URL}${slug}`;

const KelownaLocation = () => (
  <>
    <Helmet>
      <title>{title}</title>
      ...
      <section className="border-y border-border bg-card/40 py-20 md:py-28">
        <div className="container max-w-4xl">
          ...
          <h2 className="font-display text-3xl font-semibold tracking-tight text-foreground sm:text-4xl md:text-5xl">
            Mobile detailing across every Kelowna neighbourhood.
          </h2>
          <p className="mt-6 max-w-2xl text-muted-foreground">
            From lakefront homes in Lower and Upper Mission to family driveways in Glenmore and Rutland, our mobile unit comes to you.
          </p>
          <ul className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
            {KELOWNA_NEIGHBOURHOODS.map((n) => (
              <li
                key={n}
...
export default KelownaLocation;
